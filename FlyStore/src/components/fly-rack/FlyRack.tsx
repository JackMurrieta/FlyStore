...
export function useFlyRack(): RackState {
  ...
  const onMove = (x: number) => {
    ...
    if (activeEl.current)
      activeEl.current.style.transform = `translateX(${drag.current.dx}px) rotate(${drag.current.dx / 22}deg)`;
  };
  const onEnd = () => {
    if (!drag.current.on) return;
  ...
  const rackHandlers: React.HTMLAttributes<HTMLDivElement> = {
    onMouseDown: (e) => {
      if (!(e.target as HTMLElement).closest(".tag-cta")) onStart(e.clientX);
    },
    ...
    <div className="rack-wrap" {...rackHandlers}>
      {cats.map((cat, i) => {
        ...
        return (
          ...
          >
            <div className="tag-motif" aria-hidden="true">
              {cat.motif}
            </div>
...
export function FlyRackControls({
  cats,
  active,
  go,
}: Pick<RackState, "cats" | "active" | "go">) {
  return (
    <>
